...
// 3) Agregar jugadores (se valida no repetir ids)
[tigres, leones].forEach(eq => {
    eq.agregarJugador(j1); // Ana en ambos? NO: probamos duplicados por equipo
});
// El código anterior agregaría al mismo jugador a ambos equipos; esto es válido
// porque la restricción de "duplicados" se controla por equipo (no global).
// Ahora agregamos distintos jugadores por equipo y probamos duplicado DENTRO del mismo equipo.
...
try {
    ...
    console.log("Esperado (duplicado en equipo):", e.message);
}
// Halcones (básquet)
halcones.agregarJugador(j4);
halcones.agregarJugador(j5);
halcones.agregarJugador(j6);
// 4) Deportes (polimorfismo en validar)
...
for (const linea of torneo.listarPartidos())
    console.log(" - ", linea);
